...
const filePath = path_1.default.join(__dirname, '/configs', '/guardline-server.conf');
const test = () => __awaiter(void 0, void 0, void 0, function* () {
    const version = yield wireguard_tools_1.checkWgIsInstalled();
    console.log(version);
    try {
        // make a new config
        const config1 = new wireguard_tools_1.WgConfig({
            wgInterface: {
                address: ['10.10.1.1']
            },
            filePath
        ...
        const keypair = yield config1.generateKeys({ preSharedKey: true });
        ...
        yield config1.writeToFile();
        // read that file into another config object
        const thatConfigFromFile = yield wireguard_tools_1.getConfigObjectFromFile({ filePath });
        const config2FilePath = path_1.default.join(__dirname, '/configs', '/guardline-server-2.conf');
        const config2 = new wireguard_tools_1.WgConfig(Object.assign(Object.assign({}, thatConfigFromFile), { filePath: config2FilePath }));
        ...
        wireguard_tools_1.createPeerPairs([
            {
                config: config1,
        ...
        const wgVersion = yield wireguard_tools_1.checkWgIsInstalled();
        ...
        yield config1.writeToFile();
        yield config1.up(); // Wireguard interface is up
